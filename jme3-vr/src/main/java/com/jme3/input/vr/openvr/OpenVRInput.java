 ...
 * <a href="http://www.seinturier.fr">http://www.seinturier.fr</a>
 */
...
//                if( needsNewVelocity[index] ) {
                HmdVector3 tempVec = ((OpenVR) environment.getVRHardware()).hmdTrackedDevicePoses[index].vVelocity();
    ...
    public Vector3f getAngularVelocity(int controllerIndex) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVR) {
                ...
                int index = OpenVRInput.controllerIndex[controllerIndex];
                HmdVector3 tempVec = ((OpenVR) environment.getVRHardware()).hmdTrackedDevicePoses[index].vAngularVelocity();
//                    needsNewVelocity[index] = false;
//                }
                tempVel.x = tempVec.v(0);
                tempVel.y = tempVec.v(1);
    ...
    public boolean isInputDeviceTracking(int index) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVR) {
                return ((OpenVR) environment.getVRHardware()).hmdTrackedDevicePoses[controllerIndex[index]].bPoseIsValid();
            } else {
                throw new IllegalStateException("VR hardware " + environment.getVRHardware().getClass().getSimpleName() + " is not a subclass of " + OpenVR.class.getSimpleName());
            }
        } else {
    ...
    public void triggerHapticPulse(int controllerIndex, float seconds) {
        ...
        VRSystem.VRSystem_TriggerHapticPulse(OpenVRInput.controllerIndex[controllerIndex],
